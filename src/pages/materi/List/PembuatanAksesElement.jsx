...
const CodeEditorEditable = ({ title, pyodideReady, runPythonCode, onValidation, initialCode = "" }) => {
  ...
  const handleRun = useCallback(async () => {
    ...
    if (onValidation) {
      onValidation({ isValid: isValid, isComplete: isComplete, code: localCode });
    }
...
const CodeCompletionQuestion = ({ question, codeParts, placeholders, expectedAnswers, explanation, index, onCorrectChange, praktikumSelesai }) => {
...
const GuessOutputQuestion = ({ question, codeSnippet, expectedOutput, explanation, index, onCorrectChange, praktikumSelesai }) => {
...
export default function PembuatanAksesElement() {
  ...
  // Fetch progres_belajar dari Firestore & data praktikum (hanya untuk mahasiswa)
  useEffect(() => {
    if (!userId) return;
    // Jika role dosen, tidak perlu fetch progres dan redirect
    if (userRole === 'dosen') {
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (praktikSnap.exists()) {
          ...
          const code = praktikData.pembuatan_akses_element || "";
          if (code.trim() !== "") {
            setSavedCode(code);
...
print(angka[-2])`;
  ...
  // Tampilkan modal hanya jika role mahasiswa dan progres < 3 dan allCorrect
  useEffect(() => {
    if (!userId) return;
    if (userRole === 'dosen') {
      setShowModal(false);
  ...
  }, [allCorrect, userId, showModal, progresBelajar, userRole]);

  const handleCompleteAndNavigate = async () => {
    try {
      // Tambah progres hanya jika mahasiswa
      if (userRole === 'mahasiswa' && progresBelajar < 3) {
        const mahasiswaRef = doc(db, "mahasiswa", userId);
    ...
    <>
      ...
      <div className="main-content" style={{ paddingTop: "64px" }}>
        <div style={styles.page}>
          ...
          {/* ===== PAGINATION ===== */}
          <MateriPagination nextDisabled={userRole === 'mahasiswa' && progresBelajar !== null && progresBelajar < 3} />

        </div>
      </div>

      {/* Modal Sukses - HANYA UNTUK MAHASISWA DENGAN PROGRES < 3 */}
      {showModal && (
